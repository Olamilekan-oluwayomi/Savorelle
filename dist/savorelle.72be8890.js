...
════════════════════════════════════════════════════ */ function initSlider({ trackId, prevId, nextId, dotsId, cardClass = "menu__card", dotClass = "menu__dot", gap = 15, breakpoint = 768 }) {
    // Grab elements
    ...
    let currentPage = 0;
    /* --- Wishlist toggle (menu cards only — safe to run on chef cards too, just won't find any) --- */ cards.forEach((card)=>{
        const wishlistBtn = card.querySelector(".menu__card-wishlist");
        if (wishlistBtn) wishlistBtn.addEventListener("click", ()=>{
            wishlistBtn.classList.toggle("active");
        });
    });
    /* --- How many cards visible per page --- */ function getCardsPerPage() {
        return window.innerWidth >= breakpoint ? 3 : 1;
    }
    /* --- Total number of pages --- */ function getTotalPages() {
        return Math.ceil(cards.length / getCardsPerPage());
    }
    /* --- Build dot indicators --- */ function buildDots() {
    ...
    /* --- Slide to a specific page --- */ function goToPage(page) {
    ...
    /* --- Arrow buttons --- */ prev.addEventListener("click", ()=>goToPage(currentPage - 1));
    next.addEventListener("click", ()=>goToPage(currentPage + 1));
    /* --- Resize handler --- */ let previousCardsPerPage = getCardsPerPage();
...
//# sourceMappingURL=savorelle.72be8890.js.map
